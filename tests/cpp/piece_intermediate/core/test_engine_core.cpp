...
public:
    MOCK_METHOD(std::unique_ptr<Piece::RAL::IGraphicsDevice>, CreateGraphicsDevice, (Piece::WAL::IWindow* window, const NativeVulkanOptions* options), (override));
};

class MockPhysicsWorldFactory : public Piece::Intermediate::IPhysicsWorldFactory {
public:
    MOCK_METHOD(std::unique_ptr<Piece::PAL::IPhysicsWorld>, CreatePhysicsWorld, (const NativePhysicsOptions* options), (override));
};
...
protected:
    void SetUp() override {
        ...
        Piece::Intermediate::ServiceLocator::Get().SetWindowFactory(std::unique_ptr<MockWindowFactory>(window_factory_mock));
        Piece::Intermediate::ServiceLocator::Get().SetGraphicsDeviceFactory(std::unique_ptr<MockGraphicsDeviceFactory>(graphics_factory_mock));
        Piece::Intermediate::ServiceLocator::Get().SetPhysicsWorldFactory(std::unique_ptr<MockPhysicsWorldFactory>(physics_factory_mock));
    }
