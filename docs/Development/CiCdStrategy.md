...
*   **Responsibilities:**
    ...
    3.  **Packaging:** This phase is responsible for creating the final packages:
        *   **Standalone Editor:** Uses `dotnet publish` to create "zip-and-run" packages for each platform, bundling the C# host with the necessary C++ binaries (whose inclusion is orchestrated by custom MSBuild target files).
