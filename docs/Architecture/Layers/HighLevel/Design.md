...
*   **Dedicated Interop Projects:** All `unsafe` code and direct P-Invoke calls will reside within dedicated C# wrapper projects (e.g., `Piece.Glfw`, `Piece.OpenGL`, `Piece.Box2d`). The previous `Piece.Core.Interop` project has been dissolved, with its responsibilities now distributed among these specialized wrapper projects.
